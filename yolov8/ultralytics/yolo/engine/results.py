...
class Results:
    ...
    def plot(self, show_conf=True, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc', attr_list=[]):
        """
        ...
        if boxes is not None:
            ...
            for d in reversed(boxes):
                ...
                if len(attr_list)==0:
                    ...
                    attr = attr_list[i]
                    i += 1
                label = (f'{names[c]}' if names else f'{c}') + (f'{conf:.2f}' if show_conf else '') + (f" ({attr})" if attr else "")
